...
--- Tables
--- 	speedflow_15min: flow and speed (not always available) for loops in the Utrecht region as averaged over each hour. 
---			Breakdowns of flow and speed per vehicle weight category are included as separate columns.

drop table if exists rws_clean.flow_15min;
drop table if exists rws_clean.speed_15min;

--create duplicate of both speed and flow in the clean dataset
CREATE TABLE rws_clean.flow_15min AS SELECT 
...
"ROADNUMBER",
"carriageway"
FROM rws_raw.flow_15min_resolution;


CREATE TABLE rws_clean.speed_15min AS SELECT 
"measurementSiteReference",
"periodEnd",
"standardDeviation",
"dataError",
...
drop table if exists rws_clean.speedflow_15min;

CREATE TABLE rws_clean.speedflow_15min(
...
insert into rws_clean.speedflow_15min
select 
...
drop table rws_clean.flow_15min;


-- add hectokey and loop_unique_key	
alter table rws_clean.speedflow_15min 
...
update rws_clean.speedflow_15min
set dvk_or_dir = 
	case 
...
--CREATE INDEX flow_hect_link_loop_unique_key_idx ON rws_clean.flow_hect_link (loop_unique_key) ;
CREATE INDEX speedflow_15min_loop_unique_key_idx ON rws_clean.speedflow_15min (loop_unique_key) ;


update rws_clean.speedflow_15min as speedflow
