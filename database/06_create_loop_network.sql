--- This script links each flow loop to both the loop behind it, and the loop in front of it in terms of traffic flow
...
--- 	rws_clean.flow_network: for each loop, shows both the loop_id (unique per hectopoint, but not unique across lanes) and the hectokey,
---			and the loop_id and hectokey for (1) the loop ahead of the current loop in terms of traffic flow and 
---			(2) the loop behind of the current loop in terms of traffic flow
